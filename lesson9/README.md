...
```bash
 grep "error" -i -r > /Users/dima/IdeaProjects/school57kotlin/errors.txt
```
...
```bash
du -h lesson9/logs/* > lesson9/logs_size.txt
```
...
```bash
find logs/ -type f -exec du -sh {} \; | sort -h | head -1 > biglog.txt
```
...
```bash
zip -r project_backup.zip config/ logs/ errors.txt
```
